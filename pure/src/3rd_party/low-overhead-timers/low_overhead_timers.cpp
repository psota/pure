...
// booted in a 64-bit unsigned integer.
//       For all recent Intel processors, this counter increments at a fixed
//       rate, independent of the actual core clock speed or the energy-saving
//       mode.
// rdtscp() is the same as rdtsc except that it is partially ordered -- it will
// not execute until all prior
...
// unsigned integer and also
//       modifies its two integer arguments to show the processor socket and
//       processor core that were in use when the call was made.  (Note: the
//       various cores in a chip usually have very similar values for the TSC,
//       but they are allowed to vary by processor.  This function guarantees
...
// number in an integer variable. get_socket_number() uses the RDTSCP
// instruction, but returns only the socket number in an integer variable.
// rdpmc_instructions() uses a "fixed-function" performance counter to return
// the count of retired instructions on
...
// the count of actual CPU core cycles
...
// return the count of "reference" (or "nominal")
...
// instruction to get the "nominal"
//       frequency of the processor, which is also the invariant TSC frequency,
//       and returned as a float value in Hz. This can then be used to convert
//       TSC cycles to seconds.
//
...
// Utility routine to compute counter differences taking into account
// rollover when the performance counter width is not known at compile time.
...
unsigned long corrected_pmc_delta(unsigned long end, unsigned long start,
    ...
    if ((pmc_width <= 0) || (pmc_width > 64)) {
        fprintf(stderr,
                "ERROR: corrected_pmc_delta() called with illegal "
                "performance "
                "counter width %d\n",
                pmc_width);
        return (error_return);
        ...
        // sizeof(unsigned long), the simple calculation (end-start) gives the
        // correct delta even if the counter has rolled (leaving end < start).
        if (pmc_width == 64) {
            return (end - start);
        } else {
            // for pmc_width < sizeof(unsigned long), rollover must be detected
            // and corrected explicitly
            if (end >= start) {
                result = end - start;
            } else {
                // I think this works independent of ordering, but this makes
...
float get_TSC_frequency() {
    ...
    // printf("\n");
    // printf("\n");
    // printf("Scanning backwards to try to find the frequency
    // digits....\n");
        ...
        if (buffer[base] == 0x7a) {
            ...
            if (buffer[base - 1] == 0x48) {
                ...
                if (buffer[base - 2] == 0x47) {
                    ...
                    i    = base - 3;
                    stop = base - 3;
                    // printf("begin reverse search at stop character
                    // location %d\n",i);
                    while (buffer[i] != 0x20) {
                        // printf("found a non-blank character %c (%x) at
                        // location %d\n",buffer[i],buffer[i],i);
                        i--;
                    }
                    start  = i + 1;
                    ...
                    // k--;
                    // }
                    // printf("DEBUG: starting position of frequency string
                    // is %d\n",start);
                    //
                    // note that sscanf will automatically stop when the
                    // string changes from digits to non-digits, so I don't
                    // need to NULL-terminate the string in the buffer.
                    //
