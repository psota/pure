...
//       rate, independent of the actual
...
//       processor core that were in use
//       when the call was made.  (Note: the various cores in a chip usually
//       have very similar values for
//       the TSC, but they are allowed to vary by processor.  This function
//       guarantees that you know exactly
...
//       rate as the TSC, but does not count
//       when the core is in the "HALT" state.  If a timed section of code shows
//       a larger change in TSC than in
...
//       and returned as a float value in Hz.
//       This can then be used to convert TSC cycles to seconds.
//

#ifndef LOW_OVERHEAD_TIMERS_H
#define LOW_OVERHEAD_TIMERS_H
...
// unsigned long corrected_pmc_delta(unsigned long end, unsigned long start, int
// pmc_width);
...
inline __attribute__((always_inline)) unsigned long full_rdtscp(int* chip,
                                                                int* core) {
#ifdef OSX
    return 0;
#else
...
unsigned long corrected_pmc_delta(unsigned long end, unsigned long start,
    ...
    if ((pmc_width <= 0) || (pmc_width > 64)) {
        fprintf(stderr, "ERROR: corrected_pmc_delta() called with illegal "
                        "performance counter width %d\n",
                pmc_width);
        return (error_return);
    }
    // Due to the specifics of unsigned arithmetic, for pmc_width ==
    // sizeof(unsigned long),
    // the simple calculation (end-start) gives the correct delta even if the
    // counter has
    ...
    if (pmc_width == 64) {
        ...
        if (end >= start) {
            ...
            // I think this works independent of ordering, but this makes the
        ...
        if (buffer[base] == 0x7a) {
            ...
            if (buffer[base - 1] == 0x48) {
                ...
                if (buffer[base - 2] == 0x47) {
                    ...
                    i    = base - 3;
                    stop = base - 3;
                    // printf("begin reverse search at stop character location
                    // %d\n",i);
                    while (buffer[i] != 0x20) {
                    ...
                    // %d\n",start);
                    //
                    // note that sscanf will automatically stop when the string
                    // changes from digits
                    // to non-digits, so I don't need to NULL-terminate the
